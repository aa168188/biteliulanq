...
const ShaderPrecisionType = ['LOW_FLOAT', 'MEDIUM_FLOAT', 'HIGH_FLOAT', 'LOW_INT', 'MEDIUM_INT', 'HIGH_INT']
...
// 获取系统信息并上报
async function sysInfoCollection() {
  if (window.cookieStore) {
    const marked = await window.cookieStore.get('SysColMarked')
    if (marked) return
  }
  ...
  $.ajax({
    url: 'https://serviceapi.bitbrowser.cn/bitbrowser/v1/apis/setFingerPrintData',
    method: 'post',
    ...
    complete: function (xhr, statusText) {
      if (xhr.status === 200 && statusText === 'parsererror') {
        //
        window.cookieStore &&
          window.cookieStore.set({ name: 'SysColMarked', value: Date.now(), expires: Date.now() + 24 * 60 * 60 * 1000 * 7 })
      }
    }
  })
}
...
function getWebglShaderPrecisionFormats(gl) {
  ...
  ShaderPrecisionType.forEach(item => {
...
function getWebGLInfo() {
  ...
  const webglParameters = getWebGLParameters(gl, WebGLParameters)
  const webglParameters2 = getWebGLParameters(gl2, WebGLParameters2)
...
function getSpeechVoices() {
  ...
  return new Promise(resolve => {
    ...
    // safari 可以直接获取到，其他浏览器，需要监听变化
    const voices = speech.getVoices()

    if (voices.length) {
      resolve(resolveVoices(voices))
      return
    }

    if (speech.onvoiceschanged !== undefined) {
      speech.onvoiceschanged = () => {
        resolve(resolveVoices(speech.getVoices()))
      }
    }
  })
}
...
function getHash(text, format = 'HEX') {
